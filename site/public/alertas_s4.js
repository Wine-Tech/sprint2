...
// }
function alertar_s4(temperatura,umidade, idLocal) {
...
// var temperatura = resposta[0].temperatura;
    ...
    if (temperatura >= limiteTemperatura.criticoQuente) {
        ...
        exibirAlerta_s4(temperatura, idLocal, grauDeAviso, grauDeAvisoCor)
        divTempAlerta_s4.style.background = 'rgb(255, 0, 0)'
    }
        ...
        // valorCritico = true
        exibirAlerta_s4(temperatura, idLocal, grauDeAviso, grauDeAvisoCor)
        divTempAlerta_s4.style.background = 'rgb(253, 148, 0)'
        ...
        // valorCritico = true
        exibirAlerta_s4(temperatura, idLocal, grauDeAviso, grauDeAvisoCor)
        divTempAlerta_s4.style.background = 'rgb(63, 151, 210)';
    }
        ...
        // valorCritico = true
        exibirAlerta_s4(temperatura, idLocal, grauDeAviso, grauDeAvisoCor)
        divTempAlerta_s4.style.background = 'rgb(54, 44, 238)'
    }
    ...
    if (umidade >= limiteUmidade.criticoUmidadeAlta) {
        ...
        // valorCritico = true
        exibirAlerta1_s4( umidade, idLocal, grauDeAviso, grauDeAvisoCor)
        divUmidAlerta_s4.style.background = 'rgb(255, 0, 0)'
        ...
        // valorCritico = true
        exibirAlerta1_s4( umidade, idLocal, grauDeAviso, grauDeAvisoCor)
        divUmidAlerta_s4.style.background = 'rgb(63, 151, 210)'
        ...
        // valorCritico = true
        exibirAlerta1_s4( umidade, idLocal, grauDeAviso, grauDeAvisoCor)
        divUmidAlerta_s4.style.background = 'rgb(54, 44, 238)'
    }
...
function exibirAlerta_s4(temperatura, idLocal, grauDeAviso, grauDeAvisoCor) {
    var indice = alertas_s4.findIndex(item => item.idLocal == idLocal);
    console.log('cheguei',indice);
    if (indice >= 0) {
        alertas_s4[indice] = { idLocal, temperatura, grauDeAviso, grauDeAvisoCor }
    } else {
        alertas_s4.push({ idLocal, temperatura, grauDeAviso, grauDeAvisoCor });
    }
...
function exibirAlerta1_s4(umidade, idLocal, grauDeAviso, grauDeAvisoCor) {
    var indice2 = alertas_s4.findIndex(item => item.idLocal == idLocal);
    console.log('cheguei',indice2);
    if (indice2 >= 0) {
        alertas_s4[indice2] = { idLocal, umidade, grauDeAviso, grauDeAvisoCor}
    } else {
        alertas_s4.push({ idLocal, umidade, grauDeAviso, grauDeAvisoCor});
    }
    exibirCards1_s4()
}

function removerAlerta_s4(idLocal) {
    alertas_s4 = alertas_s4.filter(item => item.idLocal != idLocal);
    exibirCards_s4();
}
function removerAlerta1_s4(idLocal) {
    alertas_s4 = alertas_s4.filter(item => item.idLocal != idLocal);
    exibirCards1_s4();
}

function exibirCards_s4() {
    alerta2.innerHTML = '';

    for (var i = 0; i < alertas_s4.length; i++) {
        var mensagem2 = alertas_s4[i];
        ...
        alerta2.innerHTML += transformarEmDiv_s4(mensagem2);
    }
}
function exibirCards1_s4() {
    alerta3.innerHTML = '';

    for (var i = 0; i < alertas_s4.length; i++) {
        var mensagem3 = alertas_s4[i];
        divNotificacao.innerHTML = `
        <div class="notific">Alerta!<br> Existem armazéns que precisam de atenção.<br>Clique nesta notificação para visualizar
        <div class="alarme-sino"></div><div>`
        alerta3.innerHTML += transformarEmDiv2_s4(mensagem3);
    }
}

function transformarEmDiv_s4({ idLocal, temperatura, grauDeAviso, grauDeAvisoCor }) {
    return `<div id="divTempAlerta_s4" class="mensagem-alarme">
    <div id="infTempAlerta_s4" class="informacao">
...
function transformarEmDiv2_s4({idLocal, umidade,grauDeAviso,grauDeAvisoCor}) {
    return `<div id="divUmidAlerta_s4" class="mensagem-alarme1">
    <div id="infUmidAlerta_s4" class="informacao"> 
